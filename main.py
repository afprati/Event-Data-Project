...
instructions = """I will give a title, publication date, and text of newspaper articles. Tell me four things in this order, with a date-location pair for each event:
...
2. If rating is at least 5: what is the date(s) the violent event started (there might be more than one date)? Use the format “yyyy-mm-dd”. Separate by a semicolon. If the article also mentions peaceful events, do not tell me those dates. If no date is mentioned, use the publication date. For example if the article says events happened on Friday and is published on Sunday, subtract two days from the publication date. Return paired with locations. 
3. If rating is at least 5: what is the location(s) (there might be more than one location)? Be as specific as possible. Format as: “City: District”. Sometimes only a district is mentioned. Separate multiple locations with a semicolon. If the article also mentions peaceful events, do not tell me those places. Return paired with dates.
...
Types of events that most likely are NOT violent: Accidents; Peaceful protests/marches; Discussions of pervasive problems like violence against women or human trafficking; Natural disasters, such as landslides or floods; Attacks by wild animals; Public health problems or disease outbreaks; General strategy, such as laying out landmines; Events described in movies, TV shows, books, etc.; Suicides; Jail breaks; Smuggling; Drug issues; Bomb defusals"""
...
batch_data = pd.read_csv("data/df_new9.csv")
batch_num = 9
